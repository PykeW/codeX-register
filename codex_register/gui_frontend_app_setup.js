...
const runSuccessRateText = Vue.computed(() => {
  ...
  return `成功率 ${rate.toFixed(
    2
  )}% · 成功 ${plannedText} · 重试 ${retry} 次 · 总耗时 ${elapsed.toFixed(
    1
  )}s · 平均 ${avgSec.toFixed(1)}s`;
});
...
const runRetryReasonText = Vue.computed(() => {
  function shortReasonLabel(raw) {
    ...
    if (s.includes("otp") || s.includes("验证码") || s.includes("verification"))
      return "OTP 超时";
    if (
      s.includes("tls") ||
      s.includes("ssl") ||
    ...
    if (s.includes("no_balance") || s.includes("余额不足"))
      return "接码余额不足";
    if (
      s.includes("phone_country_blocked") ||
      s.includes("country_blocked") ||
      s.includes("国家受限")
    )
      return "手机号国家受限";
    if (s.includes("phone_sms_timeout") || s.includes("接码超时"))
      return "手机验证码超时";
    if (
      s.includes("add-phone") ||
      s.includes("phone_gate") ||
      s.includes("手机号")
    )
      return "手机号验证";
  ...
  const txt = rows
    ...
    .map(
      (x) =>
        `${shortReasonLabel((x && x.reason) || "")}×${Number(
          (x && x.count) || 0
        )}`
    )
    .join("；");
  if (txt) return `重试原因：${txt}`;
  return `重试原因：${shortReasonLabel(
    status.run_last_retry_reason || "未知"
  )}`;
});
...
const runSmsStatsText = Vue.computed(() => {
  ...
  return `SMS 统计：消耗 $${spent.toFixed(
    2
  )} · 余额 ${balText} · 下限 $${minBalance.toFixed(2)}`;
});
...
const smsOverviewText = Vue.computed(() => {
  ...
  const ctryText =
    ctryResolved >= 0 ? `${ctryCfg} -> ${ctryResolved}` : ctryCfg;
  const reuseText = smsState.reuse_phone ? "复用手机号: 开" : "复用手机号: 关";
  const updated = String(smsState.updated_at || "-");
  return `服务 ${svc} · 国家 ${ctryText} · 余额 ${balText} · 下限 $${minBal.toFixed(
    2
  )} · 累计消耗 $${spent.toFixed(2)} · ${reuseText} · 更新时间 ${updated}`;
});

const remoteInfoText = Vue.computed(() => {
  const providerLabel =
    normalizeRemoteAccountProvider(settingsForm.remote_account_provider) ===
    "cliproxyapi"
      ? "CLIProxyAPI"
      : "Sub2API";
...
const filteredMailboxRows = Vue.computed(() => {
  const kw = String(mailboxSearch.value || "")
    .trim()
    .toLowerCase();
  if (!kw) return mailboxRows.value;
  return mailboxRows.value.filter((row) =>
    String(row.address || "")
      .toLowerCase()
      .includes(kw)
  );
...
const mailInfoText = Vue.computed(() => {
  ...
  const provider = String(
    mailState.provider || settingsForm.mail_service_provider || "mailfree"
  );
  ...
  for (const dm of mailDomains.value) {
    regTotal += Number(
      mailDomainRegistered[String(dm || "").toLowerCase()] || 0
    );
  }
...
const mailboxPatternPreviewText = Vue.computed(() => {
  ...
  const randomLen = Math.max(
    0,
    Math.min(32, Math.floor(Number(settingsForm.mailbox_random_len || 0)))
  );
  const localPart =
    `${safePrefix}${previewRandomSuffix(randomLen)}` || "example";

  const allow = normalizeDomainList(settingsForm.mail_domain_allowlist || []);
  const cfgDomainsRaw =
    mailProviderTab.value === "cloudflare_temp_email"
  ...
  const sourceDomains = allow.length
    ? allow
    : mailDomains.value.length
    ? mailDomains.value
    : cfgDomains;
  const domain = sourceDomains.length
    ? String(sourceDomains[0] || "")
    : "domain.example";
...
const gmailAliasPreviewText = Vue.computed(() => {
  const user = String(settingsForm.gmail_imap_user || "")
    .trim()
    .toLowerCase();
  ...
  const server =
    String(settingsForm.gmail_imap_server || "imap.gmail.com").trim() ||
    "imap.gmail.com";
  const port = Math.max(1, Number(settingsForm.gmail_imap_port || 993));
  const tagLen = Math.max(
    1,
    Math.min(64, Math.floor(Number(settingsForm.gmail_alias_tag_len || 8)))
  );
  const mixGooglemail = !!settingsForm.gmail_alias_mix_googlemail;
  const sampleTag =
    tagLen <= 12 ? "x".repeat(tagLen) : `${"x".repeat(12)}(+${tagLen - 12})`;
  ...
  const aliasDomain =
    mixGooglemail && (domain === "gmail.com" || domain === "googlemail.com")
...
const selectedMailLabel = Vue.computed(() => {
  ...
  const row = mailRows.value.find(
    (x) => String(x.id || "") === String(selectedMailId.value)
  );
...
function applyDomainStats(data) {
  ...
  const selected = normalizeDomainList(
    (data && data.selected) || settingsForm.mail_domain_allowlist || []
  );
...
function remoteRowClassName(row) {
  ...
  else if (s && s !== "成功" && s !== "未测试" && s !== "未测")
    classes.push("row-test-fail");
  return classes.join(" ");
}
...
const accountFilteredRows = Vue.computed(() => {
  const kw = String(accountSearch.value || "")
    .trim()
    .toLowerCase();
...
async function confirmExportSub2Api() {
  ...
  try {
    ...
    if (missing.length) {
      ...
      message.success(
        `导出完成：${exported} 个，文件 ${fileNum} 个${openSuffix}`
      );
    }
    if (dir) {
...
const remoteColumns = [
  ...
  },
  {
    ...
    render(row) {
      const token = String((row && row.access_token) || "").trim();
      const provider = normalizeRemoteAccountProvider(
        settingsForm.remote_account_provider
      );
      const canDownload =
...
const cfDnsInfoText = Vue.computed(() => {
  const zone = cfZoneOptions.value.find(
    (x) => String(x.value || "") === String(cfZoneId.value || "")
  );
  const zname = String((zone && zone.label) || "").trim() || "-";
  const target =
    String(
      cfTargetDomain.value || settingsForm.cf_dns_target_domain || ""
    ).trim() || "-";
...
function assignConfig(cfg) {
  dashForm.num_accounts = coerceNumber(cfg.num_accounts, 1);
  dashForm.concurrency = coerceNumber(cfg.concurrency, 1);
  dashForm.sleep_min = coerceNumber(cfg.sleep_min, 5);
  dashForm.sleep_max = coerceNumber(cfg.sleep_max, 30);
  dashForm.fast_mode = !!cfg.fast_mode;
  dashForm.proxy = String(cfg.proxy || "");

  settingsForm.mail_service_provider = normalizeMailProvider(
    cfg.mail_service_provider || "mailfree"
  );
  mailProviderTab.value = settingsForm.mail_service_provider;
  settingsForm.mail_domain_allowlist = normalizeDomainList(
    cfg.mail_domain_allowlist || []
  );
  ...
  settingsForm.cf_temp_base_url = String(
    cfg.cf_temp_base_url || cfg.worker_domain || ""
  );
  settingsForm.cf_temp_mail_domains = String(
    cfg.cf_temp_mail_domains || cfg.mail_domains || ""
  );
  ...
  settingsForm.cf_worker_mail_domain_binding = String(
    cfg.cf_worker_mail_domain_binding || "MAIL_DOMAIN"
  );
  settingsForm.cf_dns_target_domain = String(
    cfg.cf_dns_target_domain || ""
  ).toLowerCase();
  if (!String(cfTargetDomain.value || "").trim()) {
    cfTargetDomain.value = String(settingsForm.cf_dns_target_domain || "")
      .trim()
      .toLowerCase();
  }
  ...
  settingsForm.cloudmail_admin_password = String(
    cfg.cloudmail_admin_password || ""
  );
  settingsForm.mail_curl_api_base = String(cfg.mail_curl_api_base || "");
  settingsForm.mail_curl_key = String(cfg.mail_curl_key || "");
  settingsForm.luckyous_api_base = String(
    cfg.luckyous_api_base || "https://mails.luckyous.com"
  );
  ...
  settingsForm.luckyous_email_type = String(
    cfg.luckyous_email_type || "ms_graph"
  );
  ...
  settingsForm.luckyous_specified_email = String(
    cfg.luckyous_specified_email || ""
  );
  ...
  settingsForm.graph_pre_refresh_before_run =
    cfg.graph_pre_refresh_before_run !== false;
  ...
  settingsForm.gmail_imap_server = String(
    cfg.gmail_imap_server || "imap.gmail.com"
  );
  settingsForm.gmail_imap_port = coerceNumber(cfg.gmail_imap_port, 993);
  settingsForm.gmail_alias_tag_len = coerceNumber(cfg.gmail_alias_tag_len, 8);
  settingsForm.gmail_alias_mix_googlemail =
    cfg.gmail_alias_mix_googlemail !== false;
  ...
  settingsForm.mailbox_random_len = coerceNumber(cfg.mailbox_random_len, 0);
  settingsForm.register_random_fingerprint =
    cfg.register_random_fingerprint !== false;
  ...
  settingsForm.flclash_controller = String(
    cfg.flclash_controller || "127.0.0.1:9090"
  );
  settingsForm.flclash_secret = String(cfg.flclash_secret || "");
  settingsForm.flclash_group = String(cfg.flclash_group || "PROXY");
  settingsForm.flclash_switch_policy = String(
    cfg.flclash_switch_policy || "round_robin"
  );
  settingsForm.flclash_switch_wait_sec = coerceNumber(
    cfg.flclash_switch_wait_sec,
    1.2
  );
  settingsForm.flclash_rotate_every = coerceNumber(cfg.flclash_rotate_every, 3);
  settingsForm.flclash_delay_test_url = String(
    cfg.flclash_delay_test_url || "https://www.gstatic.com/generate_204"
  );
  settingsForm.flclash_delay_timeout_ms = coerceNumber(
    cfg.flclash_delay_timeout_ms,
    4000
  );
  settingsForm.flclash_delay_max_ms = coerceNumber(
    cfg.flclash_delay_max_ms,
    1800
  );
  settingsForm.flclash_delay_retry = coerceNumber(cfg.flclash_delay_retry, 1);
  settingsForm.remote_test_concurrency = coerceNumber(
    cfg.remote_test_concurrency,
    4
  );
  settingsForm.remote_test_ssl_retry = coerceNumber(
    cfg.remote_test_ssl_retry,
    2
  );
  settingsForm.remote_refresh_concurrency = coerceNumber(
    cfg.remote_refresh_concurrency,
    4
  );
  settingsForm.accounts_sync_api_url = String(cfg.accounts_sync_api_url || "");
  settingsForm.accounts_sync_bearer_token = String(
    cfg.accounts_sync_bearer_token || ""
  );
  settingsForm.accounts_list_api_base = String(
    cfg.accounts_list_api_base || ""
  );
  settingsForm.remote_account_provider = normalizeRemoteAccountProvider(
    cfg.remote_account_provider || "sub2api"
  );
  settingsForm.cliproxy_api_base = String(cfg.cliproxy_api_base || "");
  settingsForm.cliproxy_management_key = String(
    cfg.cliproxy_management_key || ""
  );
  settingsForm.accounts_list_timezone = String(
    cfg.accounts_list_timezone || "Asia/Shanghai"
  );
...
function buildPayload() {
  return {
    num_accounts: coerceNumber(dashForm.num_accounts, 1),
    num_files: 1,
    concurrency: coerceNumber(dashForm.concurrency, 1),
    sleep_min: coerceNumber(dashForm.sleep_min, 5),
    sleep_max: coerceNumber(dashForm.sleep_max, 30),
    fast_mode: !!dashForm.fast_mode,
    proxy: String(dashForm.proxy || "").trim(),
    mail_service_provider: normalizeMailProvider(
      settingsForm.mail_service_provider || "mailfree"
    ),
    mail_domain_allowlist: normalizeDomainList(
      settingsForm.mail_domain_allowlist || []
    ),
    ...
    cf_worker_script: String(
      settingsForm.cf_worker_script || "mailfree"
    ).trim(),
    cf_worker_mail_domain_binding: String(
      settingsForm.cf_worker_mail_domain_binding || "MAIL_DOMAIN"
    ).trim(),
    cf_dns_target_domain: String(
      cfTargetDomain.value || settingsForm.cf_dns_target_domain || ""
    )
      .trim()
      .toLowerCase(),
    ...
    cloudmail_admin_email: String(
      settingsForm.cloudmail_admin_email || ""
    ).trim(),
    cloudmail_admin_password: String(
      settingsForm.cloudmail_admin_password || ""
    ).trim(),
    mail_curl_api_base: String(settingsForm.mail_curl_api_base || "").trim(),
    mail_curl_key: String(settingsForm.mail_curl_key || "").trim(),
    luckyous_api_base: String(
      settingsForm.luckyous_api_base || "https://mails.luckyous.com"
    ).trim(),
    luckyous_api_key: String(settingsForm.luckyous_api_key || "").trim(),
    luckyous_project_code: String(
      settingsForm.luckyous_project_code || ""
    ).trim(),
    luckyous_email_type: String(
      settingsForm.luckyous_email_type || "ms_graph"
    ).trim(),
    luckyous_domain: String(settingsForm.luckyous_domain || "").trim(),
    luckyous_variant_mode: String(
      settingsForm.luckyous_variant_mode || ""
    ).trim(),
    luckyous_specified_email: String(
      settingsForm.luckyous_specified_email || ""
    ).trim(),
    ...
    gmail_imap_server: String(
      settingsForm.gmail_imap_server || "imap.gmail.com"
    ).trim(),
    gmail_imap_port: coerceNumber(settingsForm.gmail_imap_port, 993),
    gmail_alias_tag_len: coerceNumber(settingsForm.gmail_alias_tag_len, 8),
    ...
    hero_sms_max_price: coerceNumber(settingsForm.hero_sms_max_price, 0),
    ...
    mailbox_random_len: coerceNumber(settingsForm.mailbox_random_len, 0),
    ...
    flclash_switch_policy: String(
      settingsForm.flclash_switch_policy || "round_robin"
    ).trim(),
    flclash_switch_wait_sec: coerceNumber(
      settingsForm.flclash_switch_wait_sec,
      1.2
    ),
    flclash_rotate_every: coerceNumber(settingsForm.flclash_rotate_every, 3),
    flclash_delay_test_url: String(
      settingsForm.flclash_delay_test_url || ""
    ).trim(),
    flclash_delay_timeout_ms: coerceNumber(
      settingsForm.flclash_delay_timeout_ms,
      4000
    ),
    flclash_delay_max_ms: coerceNumber(settingsForm.flclash_delay_max_ms, 1800),
    flclash_delay_retry: coerceNumber(settingsForm.flclash_delay_retry, 1),
    remote_test_concurrency: coerceNumber(
      settingsForm.remote_test_concurrency,
      4
    ),
    remote_test_ssl_retry: coerceNumber(settingsForm.remote_test_ssl_retry, 2),
    remote_refresh_concurrency: coerceNumber(
      settingsForm.remote_refresh_concurrency,
      4
    ),
    accounts_sync_api_url: String(
      settingsForm.accounts_sync_api_url || ""
    ).trim(),
    accounts_sync_bearer_token: String(
      settingsForm.accounts_sync_bearer_token || ""
    ).trim(),
    accounts_list_api_base: String(
      settingsForm.accounts_list_api_base || ""
    ).trim(),
    remote_account_provider: normalizeRemoteAccountProvider(
      settingsForm.remote_account_provider || "sub2api"
    ),
    cliproxy_api_base: String(settingsForm.cliproxy_api_base || "").trim(),
    cliproxy_management_key: String(
      settingsForm.cliproxy_management_key || ""
    ).trim(),
    accounts_list_timezone: String(
      settingsForm.accounts_list_timezone || "Asia/Shanghai"
    ).trim(),
...
function applySmsOverview(data) {
  ...
  smsState.min_balance_usd = Math.max(
    0,
    Number(d.min_balance_usd || settingsForm.hero_sms_max_price || 0)
  );
  smsState.balance_usd = Number(d.balance_usd ?? -1);
  smsState.balance_error = String(d.balance_error || "");
  smsState.spent_usd = Math.max(
    0,
    Number(d.spent_usd || status.run_sms_spent_usd || 0)
  );
  ...
  try {
    ...
    if (showSuccess) {
      if (Number(smsState.balance_usd) >= 0) {
        ...
        message.warning(
          `SMS 余额获取失败：${smsState.balance_error || "未知错误"}`
        );
      }
    }
  } catch (e) {
...
function applySmsCountryOptions(data) {
  ...
  const resolved = Number(
    (data && data.country_resolved) || smsState.country_resolved || -1
  );
  ...
  if (!current) {
    ...
    if (firstEnabled) {
      ...
      settingsForm.hero_sms_country = String(
        smsCountryOptions.value[0].value || ""
      );
    }
    return;
  }
...
async function refreshSmsCountryOptions(
  showSuccess = false,
  forceRefresh = true
) {
...
function applyFlclashProbeResult(data) {
  ...
  flclashProbeMeta.attempts = Number(
    (data && data.attempts) || rows.length || 0
  );
  flclashProbeMeta.ok = Number((data && data.ok) || 0);
  flclashProbeMeta.fail = Number((data && data.fail) || 0);
  flclashProbeMeta.candidate_total = Number(
    (data && data.candidate_total) || 0
  );
  flclashProbeMeta.blocked_hk_count = Number(
    (data && data.blocked_hk_count) || 0
  );
...
async function loadStatus() {
  ...
  status.config_blockers = Array.isArray(data.config_blockers)
    ? data.config_blockers
    : [];
  status.config_warnings = Array.isArray(data.config_warnings)
  ...
  status.run_sms_min_balance_usd = Math.max(
    0,
    Number(data.run_sms_min_balance_usd || 0)
  );
  ...
  remoteMeta.test_total = Number(
    data.remote_test_total || remoteMeta.test_total || 0
  );
  remoteMeta.test_done = Number(
    data.remote_test_done || remoteMeta.test_done || 0
  );
  remoteMeta.test_ok = Number(data.remote_test_ok || remoteMeta.test_ok || 0);
  remoteMeta.test_fail = Number(
    data.remote_test_fail || remoteMeta.test_fail || 0
  );
}
...
async function checkAppUpdate() {
  ...
  try {
    ...
    if (updateInfo.has_update) {
      message.success(
        `发现新版本：${updateInfo.latest_tag || updateInfo.latest_name}`
      );
    } else {
...
async function refreshAccounts(showSuccess = false) {
  ...
  try {
    ...
    accountInfo.file_options = Array.isArray(data.file_options)
      ? data.file_options.map((name) => ({
          label: String(name),
          value: String(name),
        }))
      : [];
    const allowed = new Map(accountRows.value.map((x) => [x.key, !!x.locked]));
    accountSelection.value = accountSelection.value.filter(
      (k) => allowed.has(k) && !allowed.get(k)
    );
...
async function fetchRemoteAll() {
  ...
  try {
    ...
    await refreshAccounts(false);
    message.success(
      `拉取完成：${Number(data.loaded || remoteMeta.loaded || 0)} 条`
    );
  } catch (e) {
    message.error(String(e.message || e));
  } finally {
...
function remoteSelectFailed() {
  ...
  remoteSelection.value = Array.from(
    new Set([...remoteSelection.value, ...keys])
  );
  ...
  try {
    ...
    const apis = Array.isArray(data.api_summary)
      ? data.api_summary
          ...
          .map(
            (x) =>
              `${String((x && x.api) || "-")}×${Number((x && x.count) || 0)}`
          )
          .join("；")
      : "";
    if (fail === 0) {
...
async function confirmRemoteDeleteAccounts() {
  const ids = Array.isArray(remoteDeleteIds.value)
    ? remoteDeleteIds.value.slice()
    : [];
  if (!ids.length) {
    message.warning("请先勾选要删除的账号");
    return;
  ...
  try {
    ...
    if (Number(data.fail || 0) === 0) {
      if (refreshOk) {
        message.success(
          `删除完成：成功 ${data.ok}` +
            (remoteDeleteAlsoLocal.value
              ? `，本地数据库删除 ${Number(data.local_deleted || 0)} 条`
              : "") +
            "，列表已自动刷新"
        );
      } else {
        message.warning(
          `删除完成：成功 ${data.ok}，但自动刷新失败，请手动点“获取列表与额度”`
        );
      }
    } else {
      if (refreshOk) {
        message.warning(
          `删除完成：成功 ${data.ok}，失败 ${data.fail}，列表已自动刷新`
        );
      } else {
        message.warning(
          `删除完成：成功 ${data.ok}，失败 ${data.fail}；自动刷新失败，请手动点“获取列表与额度”`
        );
      }
    }
  } catch (e) {
...
async function openRemoteGroupModal() {
  ...
  try {
    await saveConfig(false);
    const data = await apiRequest("/api/remote/groups", {
      method: "POST",
      body: {},
    });
    const items = Array.isArray(data.items) ? data.items : [];
    remoteGroupOptions.value = items
      .map((it) => ({
...
async function confirmRemoteGroupBulkUpdate() {
  ...
  try {
    ...
    const selectedNameMap = new Map(
      remoteGroupOptions.value.map((x) => [
        Number(x.value),
        String(x.label || ""),
      ])
    );
    const groupLabel = groupIds
      .map((id) => selectedNameMap.get(id) || `分组#${id}`)
      .join(", ");
    ...
    message.success(
      `批量分组完成：账号 ${Number(data.ok || accountIds.length)} 个`
    );
  } catch (e) {
    message.error(String(e.message || e));
  } finally {
...
async function deleteSelectedJson() {
  ...
  try {
    ...
    message.success(
      `删除完成：JSON ${data.removed_files} 个，账号行 ${data.removed_lines} 条`
    );
  } catch (e) {
    message.error(String(e.message || e));
  }
...
async function syncSelectedAccounts(targetProvider = "") {
  ...
  const provider = normalizeRemoteAccountProvider(
    targetProvider || settingsForm.remote_account_provider || "sub2api"
  );
  const label = provider === "cliproxyapi" ? "CPA" : "Sub2API";
  const selectedKeySet = new Set(accountSelection.value);
  const selectedRows = accountRows.value.filter(
    (x) => selectedKeySet.has(x.key) && !x.locked
  );
  ...
  try {
    ...
    message.success(
      `导入到${lowerLabel}结束：成功 ${data.ok}，失败 ${data.fail}`
    );
  } catch (e) {
    message.error(String(e.message || e));
  } finally {
...
async function exportSelectedCodeX() {
  ...
  try {
    ...
    if (missing.length) {
      message.warning(
        `导出完成：${exported} 个，缺失 ${missing.length} 个（${fileName}）${openSuffix}`
      );
    } else {
...
function currentMailProviderKey() {
  return normalizeMailProvider(
    mailProviderTab.value || settingsForm.mail_service_provider || "mailfree"
  );
}
...
function snapshotMailViewToCache() {
  ...
  target.domains = Array.isArray(mailDomains.value)
    ? [...mailDomains.value]
    : [];
...
function applyMailOverview(data) {
  ...
  const current = normalizeMailProvider(
    data.current || settingsForm.mail_service_provider || "mailfree"
  );
  ...
  const allowFromForm = normalizeDomainList(
    settingsForm.mail_domain_allowlist || []
  );
  ...
  mailboxSelection.value = mailboxSelection.value.filter((k) =>
    allowedMailboxKeys.has(k)
  );
...
async function refreshGraphAccountFiles(showError = false) {
  ...
  try {
    ...
    const items = Array.isArray(data.items) ? data.items : [];
    graphAccountFileOptions.value = items
      .map((x) => ({
...
function updateMailProviderByTab(nextVal) {
  ...
  if (
    mailProviderTab.value === "mailfree" &&
    mailfreePanelTab.value === "domain"
  ) {
...
function selectedCfZoneName() {
  const hit = cfZoneOptions.value.find(
    (x) => String(x.value || "") === String(cfZoneId.value || "")
  );
...
async function refreshMailfreeCfZones(showSuccess = false) {
  ...
  try {
    await saveConfig(false);
    const data = await apiRequest("/api/mail/cf/zones", {
      method: "POST",
      body: {},
    });
    ...
    if (apiZone && zoneSet.has(apiZone)) {
      ...
      cfZoneId.value = cfZoneOptions.value.length
        ? String(cfZoneOptions.value[0].value || "")
        : "";
    }
    ...
    settingsForm.cf_worker_script = String(
      (data && data.worker_script) ||
        settingsForm.cf_worker_script ||
        "mailfree"
    ).trim();
    settingsForm.cf_worker_mail_domain_binding = String(
      (data && data.worker_binding) ||
        settingsForm.cf_worker_mail_domain_binding ||
        "MAIL_DOMAIN"
    ).trim();

    const target = normalizeCfDomainValue(
      (data && data.target_domain) || settingsForm.cf_dns_target_domain || ""
    );
    ...
    if (showSuccess) {
      message.success(
        `Cloudflare 域名已刷新：${cfZoneOptions.value.length} 个`
      );
    }
  } catch (e) {
    if (showSuccess) {
...
async function refreshMailfreeCfDnsRecords(showSuccess = false) {
  ...
  try {
    ...
    const targetNow = normalizeCfDomainValue(
      cfTargetDomain.value || settingsForm.cf_dns_target_domain || ""
    );
...
function openCfDnsAddModal() {
  ...
  cfDnsAddForm.mode =
    String(cfDnsAddForm.mode || "random")
      .trim()
      .toLowerCase() === "manual"
      ? "manual"
      : "random";
  cfDnsAddForm.random_length = Math.max(
    3,
    Number(cfDnsAddForm.random_length || 8)
  );
...
async function confirmCfDnsBatchCreate() {
  ...
  const target = normalizeCfDomainValue(
    cfTargetDomain.value || selectedCfZoneName()
  );
  if (!target) {
  ...
  if (
    String(cfDnsAddForm.mode || "") === "manual" &&
    !String(cfDnsAddForm.manual_name || "").trim()
  ) {
...
function openCfDnsEditModal(row) {
  ...
  cfDnsEditForm.label = String(row.label || "")
    .trim()
    .toLowerCase();
...
async function deleteSelectedCfDnsRecords() {
  ...
  const ok = window.confirm(
    `确认删除已选 ${ids.length} 条 DNS 记录？该操作不可恢复。`
  );
  if (!ok) return;
...
async function setCfWorkerMailDomain(row) {
  ...
  try {
    settingsForm.cf_dns_target_domain = normalizeCfDomainValue(
      cfTargetDomain.value || settingsForm.cf_dns_target_domain || ""
    );
    await saveConfig(false);
...
async function syncSelectedCfDnsToMailfree() {
  ...
  const ok = window.confirm(
    `确认批量同步 ${domains.length} 个域名到 MailFree 域名池？`
  );
  if (!ok) return;
  ...
  try {
    ...
    if (fail > 0) {
      const sampleText = errorSamples.length
        ? `；示例：${errorSamples.join(" | ")}`
        : "";
      message.warning(
        `批量同步完成：成功 ${success}（已存在 ${existed}），失败 ${fail}${sampleText}`
      );
    } else {
...
async function refreshMailOverview(showSuccess = true) {
  ...
  try {
    ...
    if (showSuccess) {
      message.success(
        `邮箱概览已刷新：${mailDomains.value.length} 个域名，${mailboxRows.value.length} 个邮箱`
      );
    }
  } catch (e) {
    message.error(String(e.message || e));
...
async function loadMailboxEmails(mailbox, showError = true) {
  ...
  try {
    ...
    const selectedRow = mailRows.value.find(
      (x) => x.id === selectedMailId.value
    );
    if (!selectedRow) {
      resetMailDetail();
    }
...
function mailboxRowProps(row) {
  return {
    style:
      row && row.address === selectedMailbox.value
...
function mailRowProps(row) {
  return {
    style:
      row && row.id === selectedMailId.value
...
async function generateMailbox() {
  ...
  try {
    await saveConfig(false);
    const data = await apiRequest("/api/mail/generate", {
      method: "POST",
      body: {},
    });
...
async function deleteSelectedMailboxes() {
  ...
  const targets = mailboxRows.value
    .filter((x) => keySet.has(x.key))
    .map((x) => x.address);
  if (!targets.length) {
  ...
  try {
    ...
    if (
      selectedMailbox.value &&
      !mailboxRows.value.some((x) => x.address === selectedMailbox.value)
    ) {
      selectedMailbox.value = "";
      mailRows.value = [];
      mailSelection.value = [];
    ...
    if (Number(data.fail || 0) === 0) {
      ...
      const msg =
        `邮箱删除完成：成功 ${data.ok}，失败 ${data.fail}` +
...
async function deleteSelectedEmails() {
  ...
  try {
    ...
    if (Number(data.fail || 0) === 0) {
      ...
      const suffix = errs.length > 3 ? "；..." : "";
      const msg =
        `邮件删除完成：成功 ${data.ok}，失败 ${data.fail}` +
        (detail ? `；原因：${detail}${suffix}` : "");
      message.warning(msg);
    }
  } catch (e) {
...
async function poll() {
  try {
    ...
    if (
      ...
      ["mailfree", "cloudflare_temp_email", "cloudmail"].includes(
        normalizeMailProvider(mailProviderTab.value)
      ) &&
      pollTick % 6 === 0
    ) {
      await loadMailDomainStats();
...
Vue.watch(activeTab, async (tab) => {
  ...
  if (tab === "accounts") {
    await Promise.all([refreshAccounts(false), loadRemoteCache()]);
    return;
  }
  if (tab === "about") {
...
Vue.watch(
  () =>
    `${normalizeMailProvider(mailProviderTab.value)}:${String(
      mailfreePanelTab.value || "basic"
    )}`,
  async (sig) => {
